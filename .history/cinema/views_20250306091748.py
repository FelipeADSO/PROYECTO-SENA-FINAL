...
def ver_carrito(request):
    ...
    nombre_cliente = forms.cleaned_data['nombre']
    correo_cliente = forms.cleaned_data['email']
            ...
            mensaje_administrador = f"Nuevo pedido de {nombre_cliente} ({correo_cliente}, {telefono_cliente}). Total: ${total}.\n\nDetalles del pedido:\n"
            
            for item in carrito_items:
                mensaje_administrador += f"{item.producto.nombre} x{item.cantidad} - ${item.subtotal}\n"

            # Enviar el correo al cliente
            send_mail(
                'Confirmación de tu pedido',
                mensaje_cliente,
                settings.EMAIL_HOST_USER,  # Correo desde el que envías
                [correo_cliente],
                fail_silently=False,
            )
