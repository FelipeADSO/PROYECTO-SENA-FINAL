...
class Reserva(models.Model):
    fecha_creacion = models.DateTimeField(auto_now_add=True)  # Confirma que exista
    estado = models.CharField(max_length=20, choices=[('pendiente', 'Pendiente'), ('confirmado', 'Confirmado')])
    usuario = models.ForeignKey(User, on_delete=models.CASCADE)
    cantidad_personas = models.IntegerField(default=1)  # <-- Verifica que este campo exista
    productos = models.ManyToManyField(productos, blank=True)  # type: ignore
    personas = models.PositiveIntegerField()  # Asegúrate de que este campo exista
    fecha = models.DateField()  # Asegúrate de que este campo exista
    mensaje = models.TextField(blank=True, null=True)
    hora = models.TimeField(blank=True, null=True)

    def total_boletos(self):self.cantidad_personas * 12000
