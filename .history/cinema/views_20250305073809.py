...
def carrito(request):
    ...
    total = sum(reserva.cantidad_personas * 12000 for reserva in reservas)
    return render(request, 'carrito.html', {'reservas': reservas, 'total': total})
...
def reserva(request):
    ...
    if request.method == "POST":
        ...
        producto_seleccionados = request.POST.getlist("producto")

        # Agregar productos a la reserva
        for producto_id in producto_seleccionados:
            producto = Producto.objects.get(id=producto_id)
            reserva.producto.add(producto)
        ...
        if nombre and email and fecha and hora and personas:
            try:
                ...
                reserva.producto.set(Pelicula.objects.filter(id__in=pelicula_id))  # Asignamos las películas seleccionadas
...
def hacer_reserva(request):
    if request.method == "POST":
        ...
        reserva = Reserva.objects.create(usuario=request.user, cantidad_personas=cantidad_personas)
